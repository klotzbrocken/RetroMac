...
| `menuBar.palette` | `"grays4"` limits every theme surface to the four greys of a PowerBook 150 — `#000000`, `#555555`, `#AAAAAA`, `#FFFFFF` (System 7.1 (authentic)): the Apple menu, the Application menu, the Control Strip modules' menus, the strip itself, the volume slider and the desktop icons' labels. Every picture they show is snapped to those four on the way to the screen, with no gradient, no dither and no antialiasing; a picture that has to shrink is reduced block by block (nearest neighbour) so its 1 px lines survive. A desktop icon's selection inverts instead of taking the accent colour |
