...
final class DesktopIconsController {
    ...
    private func loadIconImage(for entry: DockThemeConfig.DesktopIconEntry, theme: ThemeBundle?, size: CGFloat) -> NSImage {
        // The Zip drive of the Retro Crashes: RetroMac's own artwork, not the theme's, because
        // the same drive sits on every Windows desktop and `iconResource` is confined to the
        // theme's own icons directory.
