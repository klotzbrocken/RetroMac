...
| `menuBar.palette` | `"grays4"` limits every theme surface to the four greys of a PowerBook 150 — `#000000`, `#555555`, `#AAAAAA`, `#FFFFFF` (System 7.1 (authentic)): the Apple menu, the Application menu, the Control Strip modules' menus, the strip's frame, the volume slider and the desktop icons' labels. Pictures — app icons, the theme's own icons, the Applications grid — are shown in greyscale with every grey there is, smoothly resampled, because icons snapped to four greys lose their shapes; the Control Strip shows the modules' own pictures (Mac OS 9 (authentic)'s, for the System 7.1 theme) in grey too, while the strip's ledge, grooves and frame keep to the four. A desktop icon's selection inverts instead of taking the accent colour |
| `menuBar.applicationMenuIconOnly` | `true` shows the front app's icon alone in the Application menu, as System 7.1 did; Mac OS 8.5 added the name beside it (the default) |
| `menuBar.balloonHelp` | `true` adds System 7's Balloon Help item (the `?` balloon) left of the Application menu; its menu points at macOS's own help |
| `dock.stripModules` | The Control Strip modules this theme shows, by id and in its own order: `appletalk`/`network`, `battery`, `sharing`, `keychain`, `mediabay`, `colours`, `resolution`, `printer`, `volume`, `soundsource`, `mirroring`, `hdspindown`, `power`, `sleep`. Absent: every module (the Mac OS 9 set). System 7.1 (authentic) takes the seven a PowerBook had |
