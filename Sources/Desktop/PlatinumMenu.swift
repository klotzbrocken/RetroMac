...
private final class PlatinumMenuView: NSView {
    ...
    /// The rows' icons in the 256 colours, converted once per menu (the conversion walks every pixel).
    private var monoIcons: [Int: NSImage] = [:]
    private func monoIcon(_ row: Int, _ icon: NSImage) -> NSImage {
        if let c = monoIcons[row] { return c }
        let pic = Mac256.icon(icon, points: 16, scale: window?.backingScaleFactor ?? 2); monoIcons[row] = pic; return pic
    }
    ...
    private var textDim: NSColor { mono ? Mac256.grey88 : NSColor(srgbRed: 0.5, green: 0.5, blue: 0.5, alpha: 1) }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        if !mono {   // the Platinum bevel; a System 7 menu is a plain black line
        ...
        for (i, it) in items.enumerated() {
            ...
            if isSeparator(it) {
                if mono {
                    Mac256.grey88.setFill(); NSRect(x: r.minX + 1, y: r.midY.rounded(), width: r.width - 2, height: 1).fill()
                    continue
                }
            ...
            if icons {
                if let icon = it.icon {
                    // In the 256 colours the icon keeps its colours on the inverted row.
        ...
        if scrolls {
            ...
            for (rect, up, on) in [(upArrowRect, true, canScrollUp), (downArrowRect, false, canScrollDown)] {
                ...
                p.close()
                (on ? NSColor.black : (mono ? Mac256.grey88 : NSColor(white: 0.6, alpha: 1))).setFill(); p.fill()
            }
        }
    }
