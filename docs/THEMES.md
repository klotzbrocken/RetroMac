...
| `menuBar.palette` | `"grays4"` limits every theme surface to the four greys of a PowerBook 150 — `#000000`, `#555555`, `#AAAAAA`, `#FFFFFF` (System 7.1 (authentic)): the Apple menu, the Application menu, the Control Strip modules' menus, the strip's frame, the volume slider and the desktop icons' labels. Pictures — app icons, the theme's own icons, the Applications grid — are shown in greyscale with every grey there is, smoothly resampled, because icons snapped to four greys lose their shapes; the Control Strip modules' own pixel art is shaded in greyscale too, while the strip's ledge, grooves and frame keep to the four. A desktop icon's selection inverts instead of taking the accent colour |
